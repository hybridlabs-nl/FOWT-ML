...
class TestUseScaledData:
    def test_use_scaled_data_default(self):
        ...
        x_train, y_train = make_regression(n_samples=200, n_features=3, noise=10, random_state=42)
        ...
        np.testing.assert_almost_equal(r2, 0.99, decimal=2,
    ...
    def test_use_scaled_data_calculate_score(self):
        ...
        x_train, y_train = make_regression(n_samples=200, n_features=3, noise=10, random_state=42)
        y_train = y_train * 1000  # pretend target is in dollars
        x_test, y_test = make_regression(n_samples=50, n_features=3, noise=10, random_state=24)
        y_test = y_test * 1000

        model.estimator.fit(x_train, y_train)
        metrics = ["r2", "neg_mean_squared_error"]
        expected_scores = model.calculate_score(x_train, x_test, y_train, y_test, metrics)
        ...
        np.testing.assert_almost_equal(
            actual_scores["r2"], expected_scores["r2"], decimal=2,
